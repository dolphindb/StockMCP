...
默认串行请求，`--pause` 控制请求间隔。数据权限及额度由用户自己的 Tushare 账户决定。默认源顺序：股票列表 → 历史名称 → 原始日行情 → 复权因子 → 每日指标 → 涨跌停 → ST → 指数信息/行情/权重 → 财报 → 复权行情。股票列表保留 L/D/P 三种状态，避免只取目前上市股票。`--codes` 是小范围验证选项，不过滤指数或行业资金流；支持按证券查询的日行情、复权因子、每日指标、限价和个股资金流会直接按证券及日期区间请求。不传时使用导入的证券集合。
...
三个季度财报表以股票、报表类型、报告期、实际公告日、公告日共同区分记录，避免同日多期财报互相覆盖；旧表迁移见部署说明。TSDB 表使用 LAST 策略对相同主键的观测更新。按日期分批构建即可完成回填；真实全市场长历史建议按月分批，避免一次把全部行情加载到 Python 内存。
