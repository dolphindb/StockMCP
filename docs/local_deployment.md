...
早期建表脚本使用 `ts_code + ann_date` 作为季度财报表的 LAST 键，会将同一股票、同一公告日的不同报告期或报表类型互相覆盖。当前三个季度表使用 `ts_code + report_type + end_date + f_ann_date + ann_date`。

安装器和新导入器会拒绝旧键，不会自动删除或重建现有财报表。已有部署应在全新实例安装后，从 Tushare 重新导入原始财报，再生成因子；仅复制旧表无法恢复已经被覆盖的数据。迁移验收通过后再切换客户端。此次变化仅涉及新入口使用的三个季度财报表，旧年度及其他选配导入脚本尚未做同等验收。

指数行情 `index_daily` 的重复策略也由 ALL 改为 LAST（键为股票指数代码和交易日），防止重复导入导致基准曲线重复。安装器同样拒绝旧策略，使用上述新实例迁移流程。其他旧选配表中仍有 ALL 策略；本次没有承诺所有旧入口均可幂等重跑。
